...
async function selectMatchWinner(matchId, teamId, teamName) {
    ...
    // Check if team is eliminated before allowing selection
    const teamElement = document.querySelector(`[data-team-id="${teamId}"][data-match-id="${matchId}"]`);
    if (teamElement && (teamElement.classList.contains('eliminated') || teamElement.classList.contains('disabled'))) {
        showToast('warning', `${teamName} kann nicht ausgewählt werden (eliminiert oder bereits maximal verwendet)`);
        return;
    }
...
async function loadAllPicksData() {
    try {
        ...
        for (const week of weeks) {
            ...
            // Only show past weeks and current week (skip future weeks)
